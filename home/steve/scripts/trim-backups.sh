...
# if there are more than 7 files whose name matches `forms_*` in the `/home/steve/mysql-backups` directory then delete all but the latest 7 files
[ $(find /home/steve/mysql-backups -maxdepth 1 -name "forms_*" | wc -l) -gt 7 ] && ls  /home/steve/mysql-backups/forms_* -t | tail -n +8 | xargs rm --
...
# if there are more than 7 files whose name matches `forms.tech-otaku*` in the `/home/steve/site-backups` directory then delete all but the latest 7 files
[ $(find /home/steve/site-backups -maxdepth 1 -name "forms.tech-otaku*" | wc -l) -gt 7 ] && ls  /home/steve/site-backups/forms.tech-otaku* -t | tail -n +8 | xargs rm --
