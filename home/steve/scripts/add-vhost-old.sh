...
sudo chown -R $pool:www-data /var/www/$1.$TLD
...
sudo cp /home/steve/templates/index.html /var/www/$1.$TLD/public_html/index.html
...
sudo cp /home/steve/templates/error.php /var/www/$1.$TLD/public_html/error.php
sudo cp /home/steve/templates/info.php /var/www/$1.$TLD/public_html/info.php
...
if grep -q "#AllowOverride All" /etc/apache2/sites-available/$1.$TLD.conf; then
	sudo sed -i 's/#AllowOverride All/AllowOverride All/g' /etc/apache2/sites-available/$1.$TLD.conf
fi

sudo tee $DIRECTORY/.htaccess <<HEREDOC
...
<Files info.php>
	...
	AuthDBMUserFile /var/www/$1.$TLD/.htdbm
	Require user chiaki
</Files>

HEREDOC

sudo cp -r /home/steve/.htpasswds/.htdbm /var/www/$1.$TLD/.htdbm

sudo chown -R $pool:www-data /var/www/$1.$TLD
sudo find /var/www/$1.$TLD/. -type d -exec chmod 750 {} +
sudo find /var/www/$1.$TLD/. -type f -exec chmod 640 {} +
...
echo ""
echo "IMPORTANT: '/var/www/$1.$TLD/public_html/info.php' is protected by HTTP Authentication."
