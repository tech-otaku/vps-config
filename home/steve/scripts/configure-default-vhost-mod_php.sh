...
sudo sed -i 's/REPLACE WITH DOMAIN/'"$DOMAIN"'.'"$TLD"' | '"$HOST"'/g' /var/www/html/index.html
sudo sed -i 's/REPLACE WITH TAG LINE/The future home of something cool/g' /var/www/html/index.html
...
if [[ $ssl =~ [A-Z] && $ssl == "Y" ]]; then
	if [[ $www =~ [A-Z] && $www == "Y" ]]; then
		if [[ $force =~ [A-Z] && $force == "Y" ]]; then
			sudo cp /home/steve/templates/-a-vhost-config-template-ssl-www-force.conf /etc/apache2/sites-available/000-default.conf
		else
			sudo cp /home/steve/templates/-b-vhost-config-template-ssl-www.conf /etc/apache2/sites-available/000-default.conf
		fi
	else
		sudo cp /home/steve/templates/-c-vhost-config-template-ssl.conf /etc/apache2/sites-available/000-default.conf
	fi
else
	if [[ $www =~ [A-Z] && $www == "Y" ]]; then
		if [[ $force =~ [A-Z] && $force == "Y" ]]; then
			sudo cp /home/steve/templates/-d-vhost-config-template-www-force.conf /etc/apache2/sites-available/000-default.conf
		else
			sudo cp /home/steve/templates/-e-vhost-config-template-www.conf /etc/apache2/sites-available/000-default.conf
		fi
	else
		sudo cp /home/steve/templates/-f-vhost-config-template.conf /etc/apache2/sites-available/000-default.conf
	fi
fi
