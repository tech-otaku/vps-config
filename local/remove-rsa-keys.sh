...
if [ $? -eq 0 ]; then 
	...
	cp ~/.ssh/known_hosts /Volumes/"${computer}"/.ssh
	echo "Copied ~/.ssh/known_hosts to /Volumes/"${computer}"/.ssh ..."

	cp ~/.ssh/known_hosts.bak /Volumes/"${computer}"/.ssh
	echo "Copied ~/.ssh/known_hosts.bak to /Volumes/"${computer}"/.ssh ..."
